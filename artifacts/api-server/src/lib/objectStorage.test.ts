...
test("object paths require both bucket and safe object segments", () => {
  ...
  for (const value of ["", "/bucket", "//photo.jpg", "/bucket/", "/bucket/../secret", "/bucket/folder//photo.jpg"]) {
    assert.throws(() => parseObjectPath(value), /bucket and a non-empty object name/);
  }
});
