...
export class PostgresPhotoRepository {
  ...
  async insertPhoto(photo) {
    const result = await this.pool.query(
      `INSERT INTO memories_photos (
        id, drive_file_id, thumbnail_drive_file_id, original_filename, mime_type,
        byte_size, width, height, content_hash, content_version, uploader_type,
        uploader_name, visibility, processing_state, created_at, updated_at
      ) VALUES (
  ...
  async listPublicPhotos({ cursor = null, limit = 24, processId = null, source = null } = {}) {
    ...
    const items = result.rows.slice(0, boundedLimit).map((row) => mapPhotoRow(row, row.process_ids));
    return { items, nextCursor: hasMore ? encodePhotoCursor(items.at(-1)) : null };
  }
  ...
  async findPublicPhoto(id) {
    ...
    return result.rows[0] ? mapPhotoRow(result.rows[0], result.rows[0].process_ids) : null;
  }
}
