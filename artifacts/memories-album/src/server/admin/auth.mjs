...
export function createAdminSessionCookie({
  ...
  return {
    value,
    header: `${ADMIN_SESSION_COOKIE}=${value}; Path=/Memories; Max-Age=${Math.floor(
      ttlMs / 1000,
...
export function clearAdminSessionCookie() {
  return `${ADMIN_SESSION_COOKIE}=; Path=/Memories; Max-Age=0; HttpOnly; Secure; SameSite=Strict`;
}
...
export function adminAuthorized(
  ...
  const expectedSignature = sign(payload, configuredToken);
