...
export function createGuestBatchManagementApi({
  ...
  return async function handleGuestBatchManagementApi(
    ...
    if (request.method === "POST" && rotateMatch) {
      ...
      const replacementHash = hashManagementToken(replacement);
      ...
      await auditRepository?.record({
        actor: `guest-batch:${batch.id}`,
        action: "upload-batch.rotate-link",
        targetType: "upload-batch",
        targetId: batch.id,
        before: { status: batch.status },
        after: { status: rotated.status },
    ...
    if (request.method === "DELETE" && photoMatch) {
      ...
      const withdrawn = await repository.trashBatchPhoto({
        batchId,
        photoId,
      ...
      await auditRepository?.record({
        actor: `guest-batch:${batch.id}`,
        action: "photo.withdraw",
        targetType: "photo",
        targetId: photoId,
        before: { visibility: "public" },
        after: { visibility: "trashed" },
        createdAt: now().toISOString(),
      });
      json(response, 200, {
        withdrawn: true,
        photoId,
        retentionDays: 7,
        restoreUntil: withdrawn.restoreUntil,
      });
      return true;
    }
