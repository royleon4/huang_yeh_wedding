...
function transformAdmin(source) {
  let code = replaceOnce(
    ...
    `        <label className="admin-check">\n          <input\n            type="checkbox"\n            checked={draft.showSummary !== false}\n            onChange={(event) => onChange({ showSummary: event.target.checked })}\n            disabled={busy}\n          />\n          在子流程上方顯示相簿名稱與介紹\n        </label>\n        <label className="admin-wide-field">\n          {draft.albumType === "message" ? "留言排列順序" : "相片排列順序"}\n          <select\n            value={draft.photoSortMode ?? "time-asc"}\n            onChange={(event) => onChange({ photoSortMode: event.target.value })}\n            disabled={busy}\n          >\n            <option value="random">隨機排序（重新載入時重新洗牌）</option>\n            <option value="time-asc">\n              {draft.albumType === "message" ? "留言時間：舊到新" : "拍攝時間：舊到新"}\n            </option>\n            <option value="time-desc">\n              {draft.albumType === "message" ? "留言時間：新到舊" : "拍攝時間：新到舊"}\n            </option>\n            <option value="name-asc">\n              {draft.albumType === "message" ? "留言內容：正序" : "照片名稱：正序"}\n            </option>\n            <option value="name-desc">\n              {draft.albumType === "message" ? "留言內容：反序" : "照片名稱：反序"}\n            </option>\n            <option value="author-asc">\n              {draft.albumType === "message" ? "留言者姓名：正序" : "作者／上傳者：正序"}\n            </option>\n            <option value="author-desc">\n              {draft.albumType === "message" ? "留言者姓名：反序" : "作者／上傳者：反序"}\n            </option>\n          </select>\n        </label>\n        <span className="admin-draft-hint">變更會由頁面底部統一儲存</span>`,
