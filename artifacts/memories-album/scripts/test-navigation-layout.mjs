...
async function waitForDocument(client) {
  ...
  throw new Error("Browser fixture did not finish loading");
}
...
function checkCase(testCase, value) {
  ...
  assert.equal(
    ...
    testCase.width,
    `${label}: unexpected CSS viewport width`,
  );
  assert.equal(
    value.viewport.height,
    testCase.height,
    `${label}: unexpected CSS viewport height`,
  );
  assert.equal(isSide, testCase.side, `${label}: wrong navigation mode`);

  assert(
    value.nav.left >= -0.5 && value.nav.right <= testCase.width + 0.5,
    `${label}: navigation leaves the horizontal viewport`,
  );
  assert(
    value.nav.top >= -0.5 && value.nav.bottom <= testCase.height + 0.5,
    `${label}: navigation leaves the vertical viewport`,
  );

  if (testCase.side) {
    assert(
      value.actualGap >= value.requiredGap - 0.75,
      `${label}: navigation/content gap is ${value.actualGap}px, expected at least ${value.requiredGap}px`,
    );
    assert(
      value.nav.right <= value.main.left + 0.5,
      `${label}: navigation overlaps visitor content`,
    );
    assert(
      closeEnough(value.main.width, 1280, 0.75),
      `${label}: side mode changed the 1280px content width`,
    );
    assert.match(
      value.shadow,
      /rgba\(31, 58, 47, 0\.12\)/,
      `${label}: side-specific outward shadow is missing`,
    );
  } else {
    const navCenter = value.nav.left + value.nav.width / 2;
    assert(
      closeEnough(navCenter, testCase.width / 2, 0.75),
      `${label}: bottom navigation is not centered`,
    );
    assert(
      value.nav.width <= 720.75,
      `${label}: bottom navigation exceeded its existing width`,
    );
  }
  ...
  if (testCase.scrollable) {
    assert(
      ...
      `${label}: tall side navigation did not keep its own scroll area`,
    );
  }
}
  ...
  try {
    ...
    for (const testCase of cases) {
      ...
      await client.send("Page.navigate", {
        url: `http://${host}:${fixturePort}/?viewport=${testCase.width}x${testCase.height}`,
      });
