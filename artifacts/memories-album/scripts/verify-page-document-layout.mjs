...
<html lang="zh-Hant">
  <head>
    ...
    <link rel="stylesheet" href="/page-document.css" />
    <style>
      ...
      <section class="process-page-document" data-render-state="ready">
        <div class="process-page-document-preview" data-document-kind="pptx">
          <div class="process-page-document-pptx-stage">
            <div class="mock-renderer-list">
              <div class="mock-renderer-slide"></div>
            </div>
          </div>
        </div>
      </section>
...
async function waitForDocument(client, marker) {
  for (let attempt = 0; attempt < 80; attempt += 1) {
    const ready = await evaluate(
      client,
      `document.readyState === 'complete' && location.search === ${JSON.stringify(marker)}`,
    );
    if (ready) return;
    await sleep(50);
  }
...
async function geometry(client) {
  return evaluate(
    ...
    `(() => {
      ...
      const documents = [...document.querySelectorAll('.process-page-document')];
      const controls = [...document.querySelectorAll('.process-page-document-toolbar a, .process-page-document-toolbar button')];
      ...
      return {
        ...
        documents: documents.map(rect),
        previews: previewGeometry,
...
function check(testCase, value) {
  ...
  assert.equal(value.viewportWidth, testCase.width, `${label}: wrong viewport; ${details}`);
  assert(
    value.bodyScrollWidth <= value.viewportWidth,
...
async function main() {
  ...
  const server = http.createServer((request, response) => {
    if (request.url === "/page-document.css") {
