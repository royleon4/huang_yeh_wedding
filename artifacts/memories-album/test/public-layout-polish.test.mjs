...
test("bottom album buttons become smaller without changing the nav shell or safe area", async () => {
  ...
  assert.match(appended, /flex-basis: min\(4\.55rem, 44%\)/);
  assert.match(appended, /min-height: 2\.95rem/);
  assert.match(appended, /padding: 0\.25rem 0\.32rem/);
  assert.match(appended, /min-height: 2\.8rem/);
