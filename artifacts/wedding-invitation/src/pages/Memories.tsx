...
function isPublicPhoto(value: unknown): value is PublicPhoto {
  if (typeof value !== "object" || value === null) return false;
  const photo = value as Partial<PublicPhoto>;
  return (
...
async function fetchAllPublicPhotos(): Promise<PublicPhoto[]> {
  const collected: PublicPhoto[] = [];
  ...
  do {
    ...
    const page = data as Partial<PublicPhotoPage>;
    if (!Array.isArray(page.photos)) {
      throw new Error("Invalid photo list");
    }

    collected.push(...page.photos.filter(isPublicPhoto));
...
export default function Memories() {
  ...
  const loadPhotos = async () => {
    setStatus("loading");
    try {
      setPhotos(await fetchAllPublicPhotos());
      setStatus("ready");
    } catch (error) {
